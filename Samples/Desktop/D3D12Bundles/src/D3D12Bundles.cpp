...
void D3D12Bundles::LoadPipeline()
{
   ...
   // Create descriptor heaps.
   {
      ...
      cbvSrvHeapDesc.NumDescriptors =
         ...
         + 1 + 2;                                           // + 1 for the SRV. + 2 for vertex/index buffer
...
void D3D12Bundles::LoadAssets()
{
   ...
   // Vertex+index buffer SRVs
   {
      CD3DX12_CPU_DESCRIPTOR_HANDLE srvVertexBufferHandle(m_cbvSrvHeap->GetCPUDescriptorHandleForHeapStart(), 1 * m_cbvSrvDescriptorSize);
      D3D12_SHADER_RESOURCE_VIEW_DESC vertexViewDesc = { DXGI_FORMAT_UNKNOWN, D3D12_SRV_DIMENSION_BUFFER, D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING };
      vertexViewDesc.Buffer.NumElements = m_vertexBufferView.SizeInBytes / m_vertexBufferView.StrideInBytes;
...
void D3D12Bundles::CreateFrameResources()
{
   ...
   CD3DX12_CPU_DESCRIPTOR_HANDLE cbvSrvHandle(m_cbvSrvHeap->GetCPUDescriptorHandleForHeapStart(), 3, m_cbvSrvDescriptorSize);    // Move past the SRV in slot 1.
   for (UINT i = 0; i < FrameCount; i++) {
