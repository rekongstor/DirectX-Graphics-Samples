   ...
   ID3D12DescriptorHeap* pCbvSrvDescriptorHeap, UINT cbvSrvDescriptorSize, ID3D12DescriptorHeap* pSamplerDescriptorHeap, ID3D12RootSignature* pRootSignature)
{
   ...
   // 1 SRV + how many CBVs we have currently. + 2 for vertex/index buffer
   UINT frameResourceDescriptorOffset = 1 + 2 + (frameResourceIndex * m_cityRowCount * m_cityColumnCount);
...
#if defined(UNINDEXED_VERTEX_INPUT)
         pCommandList->DrawInstanced(numIndices, 1, 0, 0);
#else
